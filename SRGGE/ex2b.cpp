...
int final_time5, initial_time5=time(NULL), frames5=0;
...
ex2b::ex2b(const QGLFormat &glf, QWidget *parent) : ex1(glf, parent)
{
    setFocusPolicy(Qt::StrongFocus);
}


void ex2b::initializeGL()
{
...
//    Set Framerate
    ...
    frames5++;
    final_time5=time(NULL);
    if(final_time5-initial_time5>0)
    {
        emit SetFramerate(QString::number(frames5/(final_time5-initial_time5)));
        frames5=0;
        initial_time5=final_time5;
    }
...
void ex2b::octreeVertexClustering()
{
...
//    float step_z = zDimension/level;
    ...
    int numVertCell = numVertici;

    //intialize the octree
    std::vector<std::vector<int>> oc_cell(8);
    //initiliazie position of octree
    int oc_x = 0;
    int oc_y = 0;
    int oc_z = 0;
    int depth = 1;
    ...
    //initialize a vector of integer with all the index of the vectors
    std::vector<int> iVertCell(numVertCell);
    int x = 0;
    std::generate(iVertCell.begin(), iVertCell.end(), [&]{ return x++; });

    std::vector<int>::iterator iter;
    //selecting the octree cell for each vertex
    for (iter= iVertCell.begin(); iter != iVertCell.end(); iter++ )
    {

        float x_cellDim = xDimension/depth;
        float y_cellDim = yDimension/depth;
        float z_cellDim = zDimension/depth;


        float x_d = mesh_->vertices_[*iter*3+0] - (mesh_->min_[0]  + x_cellDim*oc_x );
        float y_d = mesh_->vertices_[*iter*3+1] - (mesh_->min_[1]  + y_cellDim*oc_y );
        float z_d = mesh_->vertices_[*iter*3+2] - (mesh_->min_[2]  + z_cellDim*oc_z );

        float step_x = x_cellDim/2;
        float step_y = y_cellDim/2;
        float step_z = z_cellDim/2;
        //generate 0 or 1 depending on which part of the octree they belong to
        int x = static_cast<int>(x_d/step_x);
        int y = static_cast<int>(y_d/step_y);
        int z = static_cast<int>(z_d/step_z);
...
//        int f_id = i*3;
    ...
    std::cout<< "facce " << new_faces.size()/3 << std::endl;
    std::cout<< "vertici " << new_vertices.size()/3 << std::endl;
...
//    QRadioButton *OnAdvanced = new QRadioButton("LOD Advanced");
...
//    layout->addWidget(OnAdvanced,row,1);
    row++;
